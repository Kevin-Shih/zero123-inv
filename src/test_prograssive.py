...
@torch.no_grad()
def sample_new_input(input_im, model, sampler, precision, 
                     elevation, azimuth, radius, ddim_steps=75,
                     h=256, w=256, scale=3.0, ddim_eta=1.0):
...
def main_run(conf,
    ...
    for i, iter in enumerate(pbar, start=1):
        ...
        if i % conf.model.update_input_freq == 0:
            # start_elevation += est_elev.item()
            start_azimuth += est_azimuth.item()
            # start_radius += est_radius.item()
            est_azimuth.data = Tensor([0.0]).to(torch.float32).to(device)
            # generate new input image
            input_im = sample_new_input(original_input_im, LDModel, sampler, precision,
                                        start_elevation, start_azimuth, start_radius)
        optimizer.zero_grad()
        ...
        with torch.no_grad():
            ...
            # temp_radius= start_radius + est_radius.item()
...
if __name__ == '__main__':
    ...
    if match1 and match2:
        print(f"start_rel: elev= {rel_elev_deg}, azi= {rel_azi_deg}")
