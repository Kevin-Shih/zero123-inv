...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if self.learn_logvar:
            self.logvar = nn.parameter.Parameter(self.logvar, requires_grad=True)
...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            # print("applying model with split input params")
            ...
            # print("applying model without split input params")
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        # print(f"Diffusion model: {type(self.diffusion_model)}")
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None):
        ...
        if self.conditioning_key is None:
            ...
            # print(f"c_concat: {c_concat[0].shape}")
            xc = torch.cat([x] + c_concat, dim=1)
            out = self.diffusion_model(xc, t)
        elif self.conditioning_key == 'crossattn':
            # c_crossattn dimension:  torch.Size([8, 1, 768]) 1
            # cc dimension:  torch.Size([8, 1, 768]
            # print(f"c_crossattn: {c_crossattn[0].shape}")
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(x, t, context=cc)
        elif self.conditioning_key == 'hybrid':
            # print(f"c_concat: {c_concat[0].shape}, c_crossattn: {c_crossattn[0].shape}")
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc) # check denoise 1 step only?
        elif self.conditioning_key == 'hybrid-adm':
            assert c_adm is not None
            # print(f"c_concat: {c_concat[0].shape}, c_crossattn: {c_crossattn[0].shape}, c_adm: {c_adm.shape}")
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc, y=c_adm)
        elif self.conditioning_key == 'adm':
            print(f"c_adm: {c_adm.shape}")
            cc = c_crossattn[0]
